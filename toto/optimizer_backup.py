...
class TotoOptimizer:
    ...
    def optimize_insurance(
        ...
        # Split final output into base first + insured at the end.
        insured_ratio = 0.08 + (0.52 * insurance_strength)
        insured_target = 0
        ...
        if len(coupons) < target_lines:
            filler = self._assemble_coupons(
                ...
                matches=ordered_matches,
                target_lines=target_lines,
                diversity_strength=0.20 + 0.7 * insurance_strength,
            )
    ...
    def _extract_strategy_signals(self, match: dict, probs: dict[str, float]) -> dict[str, object]:
        ...
        public_raw = {
            "P1": _first_nonzero("public_p1", "crowd_p1", "history_p1", "bk_p1", "pool_p1"),
            "PX": _first_nonzero("public_px", "crowd_px", "history_px", "bk_px", "pool_px"),
            "P2": _first_nonzero("public_p2", "crowd_p2", "history_p2", "bk_p2", "pool_p2"),
        }
        ...
        signal_source = (
            "explicit_strategy_dict" if any(k in merged for k in ("strategy_signals", "history_signals"))
        ...
        return {
            "available": bool(merged) or (public is not None) or history_ready,
    ...
    def _apply_history_strategy(
        ...
        market_adjustment = min(1.0, max(0.0, public_bias + pool_vs_bk_gap))
        history_adjustment = min(1.0, max(0.0, history_score + history_conflict))

        competitive_contrarian = probs[_PROB_KEY[contrarian]] >= (float(profile["top_prob"]) - 0.08)
        if not competitive_contrarian:
            return {
                "decision": decision,
                "history_used": False,
        ...
        if decision_class == "high_uncertainty":
            return {
                "decision": self._decision_from_outcomes([top_outcome, contrarian]),
                "history_used": True,
                "reason": "high_uncertainty_with_history",
                "history_adjustment": history_adjustment,
                "market_adjustment": market_adjustment,
            }

        if decision_class.startswith("strong_single_"):
            if (favorite_overload >= 0.06 or upset_tendency >= 0.06) and contrarian != top_outcome:
                return {
                    "decision": self._decision_from_outcomes([top_outcome, contrarian]),
                    "history_used": True,
            ...
            if draw_underpricing >= 0.06 and probs["PX"] >= float(profile["top_prob"]) - 0.10:
                return {
                    "decision": self._decision_from_outcomes([top_outcome, "X"]),
                    "history_used": True,
        ...
        if len(decision) == 2 and contrarian not in decision and upset_tendency >= 0.08:
            return {
                "decision": self._decision_from_outcomes([contrarian, top_outcome]),
                "history_used": True,
        ...
        if pool_vs_bk_gap >= 0.08 and underdog_neglect >= 0.07 and probs["P2"] >= probs["PX"] - 0.03:
            return {
                "decision": self._decision_from_outcomes([top_outcome, "2"]),
                "history_used": True,
    ...
    def _insurance_pool(self, match: dict, decision: str, insurance_strength: float) -> list[str]:
        profile = self._decision_profile(match)
        probs = profile["probs"]
        ranked = profile["ranked"]
        ...
        second_margin_limit = 0.14 - (0.08 * insurance_strength)
        second_prob_floor = 0.28 - (0.12 * insurance_strength)
        if second_outcome != single_pick and (top_gap_second <= second_margin_limit or second_prob >= second_prob_floor):
        ...
        if strategy["available"] and insurance_strength >= 0.50:
            contrarian = str(strategy["contrarian_outcome"])
            if contrarian not in options and probs[_PROB_KEY[contrarian]] >= (top_prob - (0.10 + strategy_risk_boost)):
                options.append(contrarian)

        third_margin_limit = 0.22 - (0.12 * insurance_strength)
        if insurance_strength >= 0.70 and third_outcome not in options:
            if (
                third_prob >= (0.18 - 0.06 * insurance_strength - strategy_risk_boost)
        ...
        max_options = 2 if insurance_strength < 0.75 else 3
    ...
    def _build_insured_coupons_from_base(
        ...
        while len(insured_coupons) < target_count and attempts < attempt_cap:
            ...
            for rank, match_idx in enumerate(ranked_matches):
                ...
                # Prefer alternatives that are allowed by insured_pools in this match.
                valid_alternatives = [o for o in alternatives if o in insured_pools[match_idx]]
                if not valid_alternatives:
                    continue
                pick_idx = (line_idx + rank) % len(valid_alternatives)
        ...
        # Hard guarantee: if insurance is enabled and we still don't have enough
        # insured coupons, deterministically force minimal changes on top-risk matches.
        fallback_idx = 0
        while len(insured_coupons) < target_count and ranked_matches:
            ...
            for rank, match_idx in enumerate(ranked_matches):
                ...
                pick_idx = (fallback_idx + rank) % len(alternatives)
            ...
            fallback_idx += 1
            if fallback_idx > max(target_count * 10, 64):
                break
    ...
    def _insurance_risk_score(self, profile: dict[str, object], strategy: dict[str, object]) -> float:
        ...
        strategy_risk = (
            0.35 * float(strategy.get("favorite_overload", 0.0))
            + 0.35 * float(strategy.get("upset_tendency", 0.0))
            + 0.15 * float(strategy.get("underdog_neglect", 0.0))
            + 0.15 * float(strategy.get("pool_vs_bk_gap", 0.0))
        )
        return (0.55 * uncertainty) + (0.30 * closeness) + (0.15 * min(1.0, strategy_risk))
    ...
    def _build_run_summary(
        ...
        if history_available_count == 0:
            hs_reason = "no_history_signals_in_match_payload"
        elif history_used_count == 0:
        ...
        return {
            ...
            "history_strategy": {
                "history_strategy_available": history_available_count > 0,
                ...
                "applied_in_manual_mode_when_signals_present": history_used_count > 0,
                "reason": hs_reason,
            },
    ...
    def _build_run_summary_with_insurance(
        ...
        base_summary["history_strategy"] = {
            "history_strategy_available": history_draws_loaded_count > 0 or history_events_loaded_count > 0,
            "history_strategy_used": history_changed_decisions_count > 0 or history_changed_insurance_matches_count > 0,
    ...
    def _extract_history_stats(self, match: dict) -> dict[str, object]:
        ...
        if isinstance(history_payload, dict):
            history_state = str(history_payload.get("history_state") or "history_requested_but_empty")
    ...
    def _sort_indexes_by_risk(self, matches: list[dict]) -> list[int]:
        def risk(index: int) -> float:
            probs = self._normalised_probs(matches[index]["probs"])
            entropy = 0.0
